"use client";
import React from "react";
export default function AdvancedSpineSection() {
    return (
        <section className="w-full bg-white py-28 px-8 md:px-12 lg:px-20 text-center select-none">
            <div className="mx-auto flex flex-col items-center justify-center">
                {/* MAIN HEADINGS */}
                <div className="flex flex-col items-center justify-center text-[#545454] font-Adorage uppercase tracking-tight">
          {/* Top Big Heading: ADVANCED */}
                    <h2 className="text-[250px] font-normal leading-[0.85]">
                        ADVANCED
                    </h2>
                    {/* Sub Heading: SPINE SOLUTIONS */}
                    <h3 className="text-[120px] font-normal leading-[90px]">
                        SPINE SOLUTIONS
                    </h3>
                </div>
                {/* PARAGRAPH TEXT */}
                <p className="w-[89%] mt-8 md:mt-10 text-[35px] text-[#555555] font-Matangi-Bold text-base font-normal leading-[65px]">
                    Dr. Khaleel is dedicated to providing top-tier care for those facing challenges with their spine and musculoskeletal system. Whether you require operative or nonoperative treatment, you can count on our expertise to guide you toward recovery.
                </p>
            </div>

        </section>

    );

}